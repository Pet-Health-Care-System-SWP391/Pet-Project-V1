...
import { ref, get, onValue } from "firebase/database";
let updatedDataTeam = []; // Create a new array to store updated data
export let mockDataTeam = []; // Thêm dòng này
const fetchUsers = () => {
  const usersRef = ref(database, "users");
  ...
  onValue(
    ...
    (snapshot) => {
      if (snapshot.exists()) {
        ...
        mockDataTeam = updatedDataTeam; // Replace the entire array
        console.log(mockDataTeam); // Log the updated mockDataTeam array
        // Optionally, update the UI or any other components that depend on this data
      } else {
        console.log("No data available");
      }
    },
...
fetchUsers(); // Initialize the real-time listener
